...
import test_multi_modal_v11 # import test.py to get mAP after each epoch
...
from utils.loss import ComputeLoss, v8DetectionLoss, E2EDetectLoss
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    freeze_list = (
        opt.freeze
        if isinstance(opt.freeze, list)
        else range(opt.freeze)
        if isinstance(opt.freeze, int)
        else []
    )
    ...
    iterations = math.ceil(len(dataset) / max(opt.batch_size, nbs)) * opt.epochs
    ...
    criterion = E2EDetectLoss(model) if opt.end2end else v8DetectionLoss(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times = test_multi_modal_v11.test(data_dict,
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_multi_modal_v11.test(opt.data,
                                          batch_size=batch_size * 2,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
                                          iou_thres=0.7,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov8n_fusion_transformer_p2_p4_fast.yaml', help='model.yaml path')
    ...
    parser.add_argument('--optim', type=str, default='auto',
                        help='choose optim "Adam", "Adamax", "AdamW", "NAdam", "RAdam", "RMSProp", "SGD", "auto"')
    ...
    parser.add_argument('--freeze',type=lambda x: list(map(int, x.split(','))) if ',' in x else int(x),default=None, help='Freeze first n layers (int) or a list of layer indices (e.g., "1,2,3") during training')
    parser.add_argument('--end2end', action='store_true', help='yolo end2end mode')
