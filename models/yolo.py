...
class Detect_yolov11(nn.Module):
    ...
    def forward_end2end(self, x):
        ...
        one2one = [
            torch.cat((self.one2one_cv2[i](x_detach[i]), self.one2one_cv3[i](x_detach[i])), 1) for i in
            range(self.nl)
        ]
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/home/jia/PycharmProjects/multispectral-object-detection/models/transformer/yolov5s_fusion_transformer_vedai.yaml', help='model.yaml')
