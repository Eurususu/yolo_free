...
def test(data,
    ...
    if not training:
        ...
        val_path = data['val']
        dataloader = create_dataloader(val_path, imgsz, batch_size * 2, gs, opt,  # testloader
                                                rect=True, pad=0.5, prefix=colorstr(f'{task}: '))[0]
    ...
    # for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(testloader, desc=s)):
        ...
        imgs = imgs.half() if half else imgs
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, train_out = model(imgs, augment=augment)  # inference and training outputs
            ...
            t = time_synchronized()
            out = non_max_suppression_v11(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls)
        ...
        if plots and batch_i < 3:
            ...
            Thread(target=plot_images, args=(imgs, output_to_target(out), paths, f, names), daemon=True).start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/jia/PycharmProjects/multispectral-object-detection/runs/train/exp8/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='./data/bird_distri.yaml', help='*.data path')
